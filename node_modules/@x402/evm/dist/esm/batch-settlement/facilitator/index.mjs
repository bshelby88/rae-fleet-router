...
async function settleDeposit(signer, payment, payload, requirements, context, dataSuffix, allowedFactories = [], store = new InMemoryPendingSettlementStore()) {
  const { deposit, voucher } = payload;
  const config = payload.channelConfig;
  const payer = config.payer;
  const transferMethod = resolveDepositTransferMethod(payload, requirements);
...
var BatchSettlementEvmScheme = class {
  ...
  constructor(signer, authorizerSigner, config) {
    ...
    this.pendingStore = config?.pendingSettlementStore ?? new InMemoryPendingSettlementStore2();
  }
  /**
