...
__export(src_exports, {
  ...
  AUTH_CAPTURE_ESCROW_V1_0_ADDRESS: () => AUTH_CAPTURE_ESCROW_V1_0_ADDRESS,
  AUTH_CAPTURE_ESCROW_V1_1_ADDRESS: () => AUTH_CAPTURE_ESCROW_V1_1_ADDRESS,
  ...
  EIP3009_TOKEN_COLLECTOR_V1_0_ADDRESS: () => EIP3009_TOKEN_COLLECTOR_V1_0_ADDRESS,
  ERC3009_DEPOSIT_COLLECTOR_ADDRESS: () => ERC3009_DEPOSIT_COLLECTOR_ADDRESS,
  ExactEvmScheme: () => ExactEvmScheme,
  OPERATOR_REFUND_COLLECTOR_ADDRESS: () => OPERATOR_REFUND_COLLECTOR_ADDRESS,
  ...
  resolveAuthCaptureDeployment: () => resolveAuthCaptureDeployment,
...
var import_utils6 = require("@x402/core/utils");
...
async function updateChannelFromSettle(storage, input) {
  const { server, local } = input;
  let chargedAmount = 0n;
  if (server.chargedAmount !== void 0) {
  ...
  const depositAmount = local.depositAmount === void 0 ? void 0 : BigInt(local.depositAmount);
  if (!previous && chargedAmount === 0n && depositAmount === void 0) {
    return;
  }
  const nextChargedCumulative = BigInt(previous?.chargedCumulativeAmount ?? "0") + chargedAmount;
  if (server.chargedCumulativeAmount !== void 0) {
    if (!/^\d+$/.test(server.chargedCumulativeAmount) || BigInt(server.chargedCumulativeAmount) !== nextChargedCumulative) {
      return;
    }
  }
  const next = { ...previous ?? {} };
...
async function updateChannelAfterRefund(storage, channelKey, refundAmount) {
  ...
  const refundableBalance = previousBalance > chargedCumulativeAmount ? previousBalance - chargedCumulativeAmount : 0n;
  ...
  return {
    onPaymentResponse: async (ctx) => {
      ...
      if (settleResponse) {
        ...
        if (chargedAmount !== void 0 && typeof chargedAmount !== "string") {
          throw new Error("invalid chargedAmount: not a non-negative integer");
        }
        const channelState = settleResponse.extra?.channelState;
        await updateChannelFromSettle(deps.storage, {
          ...
          local: {
            ...
            depositAmount: isBatchSettlementDepositPayload(payload) ? payload.deposit.amount : void 0
          }
        });
        return;
      }
...
function resolveAuthCaptureDeployment(escrow) {
  ...
  if (!(0, import_viem24.isAddress)(escrow)) {
    return void 0;
  }
  const normalized = (0, import_viem24.getAddress)(escrow);
  if ((0, import_viem24.isAddressEqual)(normalized, AUTH_CAPTURE_ESCROW_V1_1_ADDRESS) || (0, import_viem24.isAddressEqual)(normalized, AUTH_CAPTURE_ESCROW_ADDRESS)) {
    return AUTH_CAPTURE_DEPLOYMENT_V1_1;
  }
  if ((0, import_viem24.isAddressEqual)(normalized, AUTH_CAPTURE_ESCROW_V1_0_ADDRESS)) {
    return AUTH_CAPTURE_DEPLOYMENT_V1_0;
  }
  return void 0;
}
var SALT_BINDING_TYPEHASH = (0, import_viem24.keccak256)(
  (0, import_viem24.toBytes)(
...
var PAYMENT_INFO_TYPEHASH = (0, import_viem25.keccak256)(
...
function hashPaymentInfo(chainId, paymentInfo, payer, escrowAddress = AUTH_CAPTURE_ESCROW_ADDRESS) {
  const paymentInfoEncoded = (0, import_viem25.encodeAbiParameters)(
    [
  ...
  const paymentInfoHash = (0, import_viem25.keccak256)(paymentInfoEncoded);
  const outerEncoded = (0, import_viem25.encodeAbiParameters)(
    [
  ...
  return (0, import_viem25.keccak256)(outerEncoded);
}
function computePayerAgnosticPaymentInfoHash(chainId, paymentInfo, escrowAddress = AUTH_CAPTURE_ESCROW_ADDRESS) {
  return hashPaymentInfo(chainId, paymentInfo, import_viem25.zeroAddress, escrowAddress);
}
async function signERC3009(signer, authorization, extra, tokenAddress, chainId) {
  const domain = {
    ...
    chainId,
    verifyingContract: (0, import_viem25.getAddress)(tokenAddress)
  };
  const message = {
    from: (0, import_viem25.getAddress)(authorization.from),
    to: (0, import_viem25.getAddress)(authorization.to),
    value: BigInt(authorization.value),
    validAfter: BigInt(authorization.validAfter),
    validBefore: BigInt(authorization.validBefore),
    nonce: authorization.nonce
...
async function signPermit2(signer, permit, chainId) {
  ...
  const message = {
    permitted: {
      token: (0, import_viem25.getAddress)(permit.permitted.token),
      amount: BigInt(permit.permitted.amount)
    },
    spender: (0, import_viem25.getAddress)(permit.spender),
...
function generateSalt() {
  ...
  return (0, import_viem25.toHex)(bytes);
}
function extraAddress(value) {
  if (!value || !(0, import_viem25.isAddress)(value)) return import_viem25.zeroAddress;
  return (0, import_viem25.getAddress)(value);
}
function isNonZeroAddress(value) {
  if (!value || !(0, import_viem25.isAddress)(value)) return false;
  return !(0, import_viem25.isAddressEqual)((0, import_viem25.getAddress)(value), import_viem25.zeroAddress);
}
...
function deriveBoundSalt(receiverAuthorizer, policy, saltNonce) {
  const encoded = (0, import_viem25.encodeAbiParameters)(
    [
      { name: "typehash", type: "bytes32" },
      { name: "receiverAuthorizer", type: "address" },
  ...
  return (0, import_viem25.keccak256)(encoded);
}
  ...
  async createPaymentPayload(x402Version, requirements, _) {
    ...
    if (assetTransferMethod === "permit2") {
      const permit2Authorization = {
        ...
        nonce: (0, import_viem26.hexToBigInt)(nonce).toString(),
