...
var import_facilitator3 = require("@x402/core/facilitator");
...
function isValidTxHash(hash) {
  return (0, import_viem.isHash)(hash) && !/^0x0+$/.test(hash);
}
...
var ErrErc20ApprovalBroadcastFailed = "erc20_approval_broadcast_failed";
...
function resolvePermit2ReceiptWaitSigner(signer, payload, context) {
...
async function settleEIP3009(signer, payload, requirements, eip3009Payload, config, context, store = new import_facilitator.InMemoryPendingSettlementStore()) {
  const payer = eip3009Payload.authorization.from;
  const signature = eip3009Payload.signature;
  if (signature) {
  ...
  try {
    const settleErc6492Data = (0, import_viem9.parseErc6492Signature)(signature);
    const {
    ...
    const settlePayload = erc6492InnerSig && erc6492InnerSig !== signature ? { ...eip3009Payload, signature: erc6492InnerSig } : eip3009Payload;
    const asset = (0, import_viem9.getAddress)(requirements.asset);
    ...
    return await waitAndReturnSettleResponse(signer, tx, payload.accepted.network, payer, {
      ...
      validateReceipt: (receipt) => {
        if (receipt.logs != null && !verifyEip3009TransferEvent(receipt.logs, asset, {
          from: (0, import_viem9.getAddress)(eip3009Payload.authorization.from),
          to: (0, import_viem9.getAddress)(eip3009Payload.authorization.to),
          value: BigInt(eip3009Payload.authorization.value)
        })) {
          return {
            success: false,
...
async function settlePermit2(signer, payload, requirements, permit2Payload, context, config, store = new import_facilitator2.InMemoryPendingSettlementStore()) {
  const payer = permit2Payload.permit2Authorization.from;
...
var ExactEvmScheme = class {
  ...
  constructor(signer, config) {
    ...
    this.pendingStore = config?.pendingSettlementStore ?? new import_facilitator3.InMemoryPendingSettlementStore();
  }
  /**
