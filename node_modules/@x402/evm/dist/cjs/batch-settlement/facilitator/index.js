...
var import_facilitator2 = require("@x402/core/facilitator");
...
var CHANNEL_STATE_POLL_MS = 2e3;
var CHANNEL_STATE_POLL_INTERVAL_MS = 150;
...
function isValidTxHash(hash) {
  return (0, import_viem2.isHash)(hash) && !/^0x0+$/.test(hash);
}
...
function depositSettlementCacheKey(payload, transferMethod) {
...
async function reconcilePendingDeposit(signer, payment, payload, requirements, context, cachedTx, store, cacheKey) {
  const { deposit, voucher } = payload;
  const config = payload.channelConfig;
  const payer = config.payer;
  const execution = await resolveDepositExecution(signer, payment, payload, requirements, context);
  ...
  return withPendingSettlementStore(
    ...
    () => waitAndReturnSettleResponse(receiptSigner, cachedTx, requirements.network, payer, {
      failedStatusReason: ErrDepositTransactionFailed,
      onSuccess: async () => {
...
async function settleDeposit(signer, payment, payload, requirements, context, dataSuffix, allowedFactories = [], store = new import_facilitator.InMemoryPendingSettlementStore()) {
  const { deposit, voucher } = payload;
  const config = payload.channelConfig;
  const payer = config.payer;
  const transferMethod = resolveDepositTransferMethod(payload, requirements);
  ...
  try {
    ...
    return await withPendingSettlementStore(
      store,
      cacheKey,
        ...
        onSuccess: async () => {
          ...
          if (unconfirmedBundleHash && !balanceConfirmed) {
            return balanceReadWithoutConfirmation ? {
              ...
              errorMessage: "extension signer returned a single transaction hash for the erc20 approval + deposit bundle, but the resulting channel balance does not reflect the deposit",
              transaction: tx,
              network: requirements.network,
              payer
            } : {
              success: false,
              errorReason: ErrSettlementPending,
              errorMessage: "extension signer returned a single transaction hash for the erc20 approval + deposit bundle and the post-deposit balance read failed, so the deposit could not be confirmed",
              transaction: tx,
              network: requirements.network,
              payer
            };
...
async function executeClaimWithSignature(signer, payload, requirements, authorizerSigner, dataSuffix) {
  ...
  try {
    ...
    return await waitAndReturnSettleResponse(signer, tx, network, void 0, {
      failedStatusReason: ErrClaimTransactionFailed
    });
  } catch (e) {
    return {
      success: false,
...
async function executeSettle(signer, payload, requirements, dataSuffix) {
  ...
  try {
    ...
    return await waitAndReturnSettleResponse(signer, tx, network, void 0, {
      failedStatusReason: ErrSettleTransactionFailed,
      onSuccess: (receipt) => {
        let amount = "";
        if (receipt.logs) {
...
async function executeRefundWithSignature(signer, payload, requirements, authorizerSigner, dataSuffix) {
  ...
  try {
    ...
    return await waitAndReturnSettleResponse(signer, tx, network, payload.channelConfig.payer, {
      failedStatusReason: ErrRefundTransactionFailed,
      onSuccess: async () => {
...
var BatchSettlementEvmScheme = class {
  ...
  constructor(signer, authorizerSigner, config) {
    ...
    this.pendingStore = config?.pendingSettlementStore ?? new import_facilitator2.InMemoryPendingSettlementStore();
  }
  /**
